...
**warprrr** streamlines data import, validation, and caching workflows in R by leveraging S7 classes and high-performance libraries (**arrow**, **data.table**, **haven**).  
It supports multiple file formats, fast caching, robust configuration, and precise timing—making it ideal for large-scale or clinical data workflows.

## Key Features

- **Unified Data Import**: Read CSV, TSV, PSV, TXT, Parquet, Feather, SAS XPT/sas7bdat formats with a consistent API.
- **Smart Caching**: Automatic feather-format caching for rapid re-reads.
- **Flexible Backend**: Forwards arguments to backend readers such as `fread`, `arrow`, `haven`.
- **File Integrity & Validation**: Validates file existence and writability before read.
- **Precise Timing & Verbosity**: Detailed logging and timing.
- **Modern R6/S7 Architecture**: Fully S7-powered for extensibility.
- **Clinical-Grade Reliability**: Built for high-volume reporting and multi-format ingestion.
...
Custom read arguments are forwarded automatically via `...` or `read_fun_args`.
